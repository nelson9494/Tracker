...
{name:'forceSettings',type:'boolean'},{name:'coordinateFormat',type:'string'},{name:'poiLayer',type:'string'},{name:'announcement',type:'string'},{name:'attributes'}],proxy:{type:'ajax',url:'api/server',actionMethods:{update:'PUT'},writer:{type:'json',writeAllFields:!0}}});
...
Ext.define('Traccar.view.MainController',{extend:Ext.app.ViewController,alias:'controller.mainController',init:function(){this.lookupReference('reportView').setHidden(Traccar.app.getPreference('disableReports',!1));this.lookupReference('eventsView').setHidden(Traccar.app.getBooleanAttributePreference('ui.disableEvents'))}});
...
name:this.keys[key].name,attribute:key,value:Traccar.AttributeFormatter.getFormatter(key)(this.position.get(key))}));var attributes=this.position.get('attributes');if(attributes instanceof Object)for(key in attributes)if(attributes.hasOwnProperty(key)&&!this.hideAttributes[key]){this.lookupAttribute=key;var name=Ext.getStore('PositionAttributes').getAttributeName(key,!0);var value=void 0!==this.position.get('attribute.'+key)?Traccar.AttributeFormatter.getAttributeFormatter(key)(this.position.get('attribute.'+
key)):Traccar.AttributeFormatter.defaultFormatter(attributes[key]);store.add(Ext.create('Traccar.model.Attribute',{priority:1024,name:name,attribute:key,value:value}))}},selectDevice:function(device){this.deviceId=device.get('id');(device=Ext.getStore('LatestPositions').findRecord('deviceId',this.deviceId,0,!1,!1,!0))?(this.position=device,this.updatePosition()):(this.position=null,Ext.getStore('Attributes').removeAll())},selectPosition:function(position){position instanceof Traccar.model.Position&&
(this.deviceId=null,this.position=position,this.updatePosition())},deselectFeature:function(){this.position=this.deviceId=null;Ext.getStore('Attributes').removeAll()},clearReport:function(){this.deviceId||(this.position=null,Ext.getStore('Attributes').removeAll())},onGeocode:function(){var positionId=this.position.getId();this.position.get('address')||Ext.Ajax.request({scope:this,method:'GET',url:'api/server/geocode',params:{latitude:this.position.get('latitude'),longitude:this.position.get('longitude')},
success:function(response){this.position&&this.position.getId()===positionId&&(this.position.set('address',response.responseText),this.updatePosition())},failure:function(response){Traccar.app.showError(response)}})}});
...
flex:1},items:[{text:Strings.stateName,dataIndex:'name'},{text:Strings.stateValue,dataIndex:'value',cellWrap:!0,renderer:function(value,metaData,record){record=record.get('attribute');if('alarm'===record)metaData.tdCls='view-color-red';else{if('address'===record&&!value)return'\x3ca href\x3d"#" onclick\x3d"Ext.fireEvent(\'stategeocode\')" \x3e'+Strings.sharedShowAddress+'\x3c/a\x3e';if('image'===record||'video'===record||'audio'===record)if(metaData=this.getController().position)if(metaData=Ext.getStore('Devices').getById(metaData.get('deviceId')))return'\x3ca target\x3d"_blank" href\x3d"/api/media/'+
metaData.get('uniqueId')+'/'+value+'" \x3e'+value+'\x3c/a\x3e'}return value}}]}});
...
Ext.define('Traccar.view.map.MapController',{extend:Traccar.view.map.MapMarkerController,alias:'controller.map',config:{listen:{controller:{'*':{mapstaterequest:'getMapState',zoomtoalldevices:'zoomToAllDevices'}},store:{'#Geofences':{load:'updateGeofences',add:'updateGeofences',update:'updateGeofences',remove:'updateGeofences'}}}},init:function(){this.callParent();this.lookupReference('showReportsButton').setVisible(Traccar.app.isMobile()&&!Traccar.app.getPreference('disableReports',!1));this.lookupReference('showEventsButton').setVisible(Traccar.app.isMobile()&&
!Traccar.app.getBooleanAttributePreference('ui.disableEvents'))},showReports:function(){Traccar.app.showReports(!0)},showEvents:function(){Traccar.app.showEvents(!0)},onFollowClick:function(button,pressed){pressed&&this.selectedMarker&&this.getView().getMapView().setCenter(this.selectedMarker.getGeometry().getCoordinates())},showLiveRoutes:function(button){this.getView().getLiveRouteLayer().setVisible(button.pressed)},showAccuracy:function(button){this.getView().getAccuracyLayer().setVisible(button.pressed)},
...
this.lookupReference('expirationTimeField').setDisabled(!1),this.lookupReference('deviceReadonlyField').setDisabled(!1),this.lookupReference('limitCommandsField').setDisabled(!1),this.lookupReference('disableReportsField').setDisabled(!1)},testNotification:function(){Ext.Ajax.request({url:'api/notifications/test',method:'POST',failure:function(response){Traccar.app.showError(response)}})},onSaveClick:function(button){var dialog=button.up('window').down('form');dialog.updateRecord();dialog=dialog.getRecord();
if(dialog===Traccar.app.getUser())dialog.save();else{var store=Ext.getStore('Users');dialog.phantom&&store.add(dialog);store.sync({failure:function(batch){store.rejectChanges();Traccar.app.showError(batch.exceptions[0].getError().response)}})}button.up('window').close()}});
...
inputValue:!0,uncheckedValue:!1,name:'registration',fieldLabel:Strings.serverRegistration},{xtype:'checkboxfield',inputValue:!0,uncheckedValue:!1,name:'readonly',fieldLabel:Strings.serverReadonly},{xtype:'checkboxfield',inputValue:!0,uncheckedValue:!1,name:'deviceReadonly',fieldLabel:Strings.userDeviceReadonly},{xtype:'checkboxfield',inputValue:!0,uncheckedValue:!1,name:'limitCommands',fieldLabel:Strings.userLimitCommands},{xtype:'checkboxfield',inputValue:!0,uncheckedValue:!1,name:'disableReports',
fieldLabel:Strings.userDisableReports}]}]},buttons:[{text:Strings.sharedAttributes,handler:'showAttributesView'},{glyph:'xf041@FontAwesome',minWidth:0,handler:'getMapState',tooltip:Strings.sharedGetMapState,tooltipType:'title'},{xtype:'tbfill'},{glyph:'xf00c@FontAwesome',tooltip:Strings.sharedSave,tooltipType:'title',minWidth:0,handler:'onSaveClick'},{glyph:'xf00d@FontAwesome',tooltip:Strings.sharedCancel,tooltipType:'title',minWidth:0,handler:'closeView'}]});
